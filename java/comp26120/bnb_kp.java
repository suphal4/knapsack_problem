...
public class bnb_kp extends KnapSack {
	...
	/* YOUR CODE GOES HERE */
	struc_sol abc = new struc_sol();
	abc.fixed=0;
	frac_bound(abc,0);
	int current_best=abc.val;
	insert(abc);
	while(QueueSize>0 && pqueue[1].bound>current_best){
		struc_sol s= removeMax();
		struc_sol c1=s.copy();
		struc_sol c2=s.copy();
		c1.fixed+=1;
		c1.solution_vec.set(c1.fixed, true);
		c2.fixed+=1;
		c2.solution_vec.set(c2.fixed, false);
		frac_bound(c1, c1.fixed);
		if (c1.val!=-1 ){
			if (c1.val>current_best){
				current_best=c1.val;
				copy_array(c1.solution_vec, final_sol);
			}
			insert(c1);
		}
		frac_bound(c2, c2.fixed);
		if (c2.val!=-1 ){
			if (c2.val>current_best){
				current_best=c2.val;
				copy_array(c2.solution_vec, final_sol);
			}
			insert(c2);
		}
	}
